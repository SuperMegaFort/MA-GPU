...
__global__ static void addScalar(int a , int b , int* ptrSumGM);
...
__host__ bool addScalar_procedurale() // __host__ facultatif
    ...
    addScalar<<<dg,db>>>(a,b,ptrSumGM); // assynchrone, call le kernel GPU addScalar
 ...
 */
__global__ void addScalar(int a , int b , int* ptrSumGM)
    {
